...
../configure \
	...
	--with-cflags='-O2   -DLOGIN_PROGRAM=\"/bin/login\" -DLOGIN_NO_ENDOPT -fvisibility=hidden -fvisibility-inlines-hidden' \
...
install -m 644 %{_sourcedir}/sshd.service %{buildroot}%{_libdir}/systemd/system/sshd.service
install -m 644 %{_sourcedir}/sshd-pre.service %{buildroot}%{_libdir}/systemd/system/sshd-pre.service
...
cat LICENSE > $RPM_BUILD_ROOT%{_datadir}/license/openssh-server
cat LICENSE > $RPM_BUILD_ROOT%{_datadir}/license/openssh-client
...
get_config_option() {
       ...
       # Tizen don't support script language
       # So perl code changed to shell script.
#       perl -lne 's/\s+/ /g; print if s/^\s*'"$option"'[[:space:]=]+//i' \
#          /etc/ssh/sshd_config
	grep -R "^$option" /etc/ssh/sshd_config  |cut -d' ' -f2
}
...
%files client
%defattr(-,root,root)
%{_datadir}/license/openssh-client
/etc/ssh/moduli
/etc/ssh/ssh_config
%{_bindir}/scp
%{_bindir}/sftp
...
%files server
%defattr(-,root,root)
%{_datadir}/license/openssh-server
/etc/default/ssh
...
%{_libdir}/systemd/system/sshd-pre.service
%manifest openssh.manifest
